...
<div class="post-content_item">
  ...
  <div class="summary-content"><a href="/manga-author/chugong/">Chugong</a></div>
</div>
<div class="post-content_item">
  <div class="summary-heading"><h5>Artist(s)</h5></div>
...
<div class="post-content_item">
  <div class="summary-heading"><h5>Author(s)</h5></div>
  <div class="summary-content"><a>Chugong</a></div>
</div>
<div class="post-content_item">
  <div class="summary-heading"><h5>Artist(s)</h5></div>
  <div class="summary-content"><a>REDICE Studio</a></div>
</div>
"""


def test_madara_extract_people_rejects_digits():
    """Year-as-artist leak from manhuaplus.com/i-am-the-fated-villain/ — the
    ...
    """
    handler = MadaraSiteHandler("manhuaplus", "https://manhuaplus.com")
...
<div class="entry-content"><p>10 years ago, after the Gate...</p></div>
<div class="wd-full">
  <div class="imptdt">Status <i>Completed</i></div>
  <div class="imptdt">Type <a>Manhwa</a></div>
  <div class="imptdt">Released <i>2018</i></div>
  <div class="imptdt">Author <a>Chugong</a></div>
  <div class="imptdt">Artist <a>REDICE Studio</a></div>
</div>
<div class="mgen">
...
def test_mangathemesia_imptdt_full_extraction():
    ...
    comic = ctx.comic
    assert comic["title"] == "Solo Leveling"
    assert "10 years ago" in (comic["desc"] or "")
    assert comic["authors"] == ["Chugong"]
    assert comic["artists"] == ["REDICE Studio"]
...
def test_extract_imptdt_values_dedupe_and_order():
    ...
    handler = _make_mt_handler()
    soup = BeautifulSoup(
        '<div class="imptdt">Author <a>Chugong</a> <a>Chugong</a> <a>h-goon</a></div>',
        "html.parser",
    )
    assert handler._extract_imptdt_values(soup, "Author") == ["Chugong", "h-goon"]
...
# MangAdventure framework typical series page (assortedscans.com style):
...
<html><head>
  ...
  <main id="content">
    ...
    <div class="chapter-list">
      <li class="chapter-details"><a href="/reader/a-game-of-murder/1/1/">Ch1</a></li>
    </div>
  </main>
</body></html>
"""

# arc-relight.com style: meta tags ARE populated (the family is heterogeneous).
...
def test_assortedscans_dom_extraction_populates_all_fields():
    ...
    ctx = handler.fetch_comic_context(
        "https://assortedscans.com/reader/a-game-of-murder/",
        MagicMock(),
...
def test_assortedscans_meta_path_still_works():
    ...
    handler = AssortedScansSiteHandler()
    ctx = handler.fetch_comic_context(
        "https://arc-relight.com/reader/test/",
        MagicMock(),
...
def test_assortedscans_og_description_fallback():
    ...
    handler = AssortedScansSiteHandler()
    ctx = handler.fetch_comic_context(
        "https://assortedscans.com/reader/x/",
        MagicMock(),
        _mr_returning(html),
    )
...
def test_flamecomics_strip_html_mantine_wrapper():
    ...
    raw = '<p class="mantine-focus-auto m_b6d8b162 mantine-Text-root">10 years ago, after "the Gate"...</p>'
    cleaned = FlameComicsSiteHandler._strip_html(raw)
    assert cleaned == '10 years ago, after "the Gate"...'
...
<div class="manga-info">
  <div class="poster"><img itemprop="image" src="https://cdn.mangafire.to/c.jpg"/></div>
  ...
  <div class="meta">
    <div><span>Author:</span> <a itemprop="author" href="/author/x">Chugong</a></div>
    <div><span>Artist:</span> <a itemprop="artist" href="/artist/x">REDICE Studio</a></div>
...
def test_mangafire_artists_itemprop_extracted():
    ...
    ctx = handler.fetch_comic_context(
        "https://mangafire.to/manga/solo-leveling.x",
        MagicMock(),
        _mr_returning(_MANGAFIRE_FULL_HTML),
    )
    assert ctx.comic["authors"] == ["Chugong"]
    assert ctx.comic["artists"] == ["REDICE Studio"]
...
def test_mangafire_artists_label_fallback():
    ...
    html = _MANGAFIRE_FULL_HTML.replace(
        '<a itemprop="artist" href="/artist/x">REDICE Studio</a>',
        '<a href="/artist/x">REDICE Studio</a>',
    )
    from sites.mangafire import MangaFireSiteHandler
    handler = MangaFireSiteHandler()
    ctx = handler.fetch_comic_context(
...
<html><head>
  ...
  <meta property="og:image" content="https://cdn.webtoons.com/cover.jpg" />
</head><body>
...
<div class="container">
  <div>
    <div><img src="https://cdn.mangapill.com/cover.jpg"/></div>
    <div>
      <div><h1>Solo Leveling</h1></div>
      <div><p>Description here.</p></div>
      <div>
        <div><div>Author</div><div><a>Chugong</a></div></div>
        <div><div>Artist</div><div><a>REDICE Studio</a></div></div>
...
def test_mangapill_artist_branch_populates():
    ...
    ctx = handler.fetch_comic_context(
        "https://mangapill.com/manga/8136/solo-leveling-novel",
        MagicMock(),
        _mr_returning(_MANGAPILL_META_HTML),
    )
    assert ctx.comic.get("authors") == ["Chugong"]
    assert ctx.comic.get("artists") == ["REDICE Studio"]
...
def test_mangapill_no_artist_row_leaves_field_absent():
    ...
    handler = MangaPillSiteHandler()
    ctx = handler.fetch_comic_context(
        "https://mangapill.com/manga/8136/test",
        MagicMock(),
        _mr_returning(html),
    )
    assert ctx.comic.get("authors") == ["Chugong"]
...
<html><body>
<h1 class="heading">Solo Leveling</h1>
<div class="cover"><img src="https://example.com/c.jpg"/></div>
<div class="summary"><p>10 years ago...</p></div>
<ul class="d39">
  <li>
    <div class="label">Authors:</div>
    <div class="value"><a href="https://mangakatana.com/authors/chugong">Chugong</a></div>
  </li>
  <li>
...
def test_mangakatana_authors_empty_when_truly_missing():
    ...
    handler = MangaKatanaSiteHandler()
    ctx = handler.fetch_comic_context(
        "https://mangakatana.com/manga/test.x",
        MagicMock(),
...
def test_details_json_dict_shape_from_handlers():
    ...
    assert details["author"] == "Chugong"
    assert details["artist"] == "REDICE Studio"
    assert details["description"].startswith("10 years ago")
